...
AllFlags ReadFlags(int argc, char **argv) {
  ...
  while ((FlagNow = getopt_long(argc, argv, "e:ivclnohsf:", 0, 0)) != -1) {
    switch (FlagNow) {
      case 'e':
        flags.Mpattern =
            Optargunion(flags.Mpattern, &pattern_size, optarg, strlen(optarg));
        break;
      case 'i': 
      ...
      case 'f':
        flags.patternfile =
            filePattern(flags.patternfile, &fpattern_size, optarg);
        break;
    }
  }
  if (pattern_size) {
...
void GrepFile(FILE *file, AllFlags flags, regex_t *preg, char *filename,
  ...
  while (getline(&line, &length, file) >
         0) {  //три аргумента: указатель на строку, в которую будет
               //записываться ввод, указатель на переменную, которая хранит
               //размер буфера строки, и файловый дескриптор ввода
    strokaCounter++;
    line[strcspn(line, "\n")] = '\0';
    if (flags.invert) { /* ЕСЛИ ФЛАГ V*/
    ...
    }  // 5 арг-указ на ском рег выраж, строку, котор проверить, кол элем в
       // массиве, указывающем на подвыражения, массив, указывающий на
       // подвыражения, и флаги, которые могут изменять поведение сопоставления
       // */
      ...
      if (!regexec(preg, line, 1, &match, 0)) {
        if (flags.printMatched) {  // Флаг -О
          if (count_files >= 2)
            printf("%s:%.*s\n", filename, match.rm_eo - match.rm_so,
                   line + match.rm_so);
          else
        ...
        else if (flags.numberLine)  // если ФЛАГ -N
  ...
  while ((getline(&line, &length, file) > 0) &&
         coincid ==
             0) {  //три аргумента: указатель на строку, в которую будет
                   ...
                   //размер буфера строки, и файловый дескриптор ввода
    line[strcspn(line, "\n")] = '\0';
    if (!regexec(preg, line, 1, &match, 0)) coincid = 1;
  }
  if (coincid == 1) {
    rewind(file);
    while (getline(&line, &length, file) >
...
void Grep(int argc, char *argv[], AllFlags flags, int count_files) {
  ...
  if (flags.Msize == 0 &&
      flags.Fsize == 0) {  //В МЕЙН интересно что будет без паттерна //
    if (regcomp(preg, argv[0],
                flags.regex_flag)) {  //а точно ли строка это argv[0]
    ...
    if (regcomp(preg, flags.patternfile + 2,
                flags.regex_flag)) {  //зачем ей флаг?
      fprintf(stderr, "faild to cimpile regex\n");
      exit(1);
    }
  } /*ДЕФОЛТНЫЙ СЛУЧАЙ*/
